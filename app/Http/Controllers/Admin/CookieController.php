        ...
        try {
            ...
            foreach (data_get($data, 'cookies', []) as $c) {
                CookieScanCookie::create([
                    ...
                    'expires'           => data_get($c, 'expiresAt'),
                    'expires_timestamp' => data_get($c, 'expiresTimestamp'),
                    'size'              => data_get($c, 'size', 0),
                    'http_only'         => (bool) data_get($c, 'httpOnly', false),
                    'secure'            => (bool) data_get($c, 'secure', false),
                    'same_site'         => data_get($c, 'sameSite'),
                    'session'           => (bool) data_get($c, 'isSession', false),
                    'type'              => data_get($c, 'category', 'functional'),
